...
class AuthenticationsHandler {
  ...
  async putAuthenticationHandler(request, reply) {
    ...
    const accessToken = this._tokenManager.generateAccessToken({ id });
    return reply.response(successResponse({ accessToken }, 'Success Update Authentication User'));
  }
  ...
  async deleteAuthenticationHandler(request, reply) {
    ...
    return reply.response(successResponse(undefined, 'Refresh token berhasil dihapus'));
  }
}
